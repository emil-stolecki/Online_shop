...
public interface UserRepository extends JpaRepository<UserModel,Long>{
	...
	@Query(value = "SELECT new "+dto3+"(u.login,u.email,u.encryptedPassword) FROM users u WHERE u.email = :email")
	Optional<UserCheckLoginDto> findLoginDtoByEmail(@Param("email")String email);
	@Query(value = "SELECT new "+dto3+"(u.login,u.email,u.encryptedPassword) FROM users u WHERE u.login = :login")
